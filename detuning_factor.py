...
with plt.style.context(['science','nature']):
    ...
    ax1.plot(delta_nva2,chip(delta_nva2,0)/np.max(chip(delta_nva2,0)))
    ax1.plot(delta_nva2,chim(delta_nva2,0)/np.max(chip(delta_nva2,0)))
    ...
    ax1.set_xticklabels([])
    ax1.text(-2, 1.15, '(a)',fontsize=8)

    ax2 = fig.add_subplot(312)
    ax2.plot(delta_nva2,chip(delta_nva2,0.5)/np.max(chip(delta_nva2,0.5)))
    ax2.plot(delta_nva2,chim(delta_nva2,0.5)/np.max(chip(delta_nva2,0.5)))
    ax2.plot(delta_nva2,np.zeros(len(delta_nva2)),linewidth=0.2,color='black',linestyle='dotted')
    ...
    ax2.set_xticklabels([])
    ax2.text(-2, 1.15, '(b)',fontsize=8)

    ax3 = fig.add_subplot(313)
    ax3.plot(delta_nva2,chip(delta_nva2,0.99)/np.max(chip(delta_nva2,0.99)))
    ax3.plot(delta_nva2,chim(delta_nva2,0.99)/np.max(chip(delta_nva2,0.99)))
    ax3.plot(delta_nva2,np.zeros(len(delta_nva2)),linewidth=0.2,color='black',linestyle='dotted')

    ax3.set_ylabel(' (arb. units)', fontsize=10)
    ax3.tick_params(axis='x', labelsize='10' )
    ax3.tick_params(axis='y', labelsize='10' )
    # ax3.set_yticks([-1,-0.5,0,0.5,1]) # 设置刻度
    ax3.text(-2, 1.15, '(c)',fontsize=8)


    plt.scatter(4.5,0,s=1,c='orange')
    plt.scatter(-2.3,0,s=1,c='red')
    plt.scatter(-3.1,0,s=1,c='purple')
    plt.scatter(3.7,0,s=1,c='black')


    ax1.legend(["$\chi_+$", "$\chi_-$"], loc='upper right',prop={'size':9})
    # plt.xlabel('Frequency (Hz)', fontsize=12)
    # plt.ylabel(' PSD ($ N \chi_+^2$/Hz)', fontsize=12)
    plt.xlabel('$\\nu-\\nu_0$ (GHz)', fontsize=10)

    plt.savefig('imag/detuning.png', dpi=1000)
plt.show()
