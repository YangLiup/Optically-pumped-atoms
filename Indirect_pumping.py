...
def master_equation(I,Rse,omega_0,T):
    a = round(I + 1 / 2)
    b = round(I - 1 / 2)
    ...
    Max = np.zeros(round(T / dt))
    Maz = np.zeros(round(T / dt))
    V = np.zeros(round(T / dt))

    projection=block_diag(np.zeros((5,5)),np.ones((3,3)))
    Syb=projection*Sy
    Szb=projection*Sz
    delta_Gamma0=Rop*block_diag(np.zeros((5,5)),np.identity(3))
    delta_Gamma1=-Rop*2*(Syb*sy+Szb*sz)
    delta_Gamma=delta_Gamma0+delta_Gamma1
    ...
    for n in trange(0, round(T / dt), 1):
        ...
        de_op = -(delta_Gamma@Rhot+Rhot@delta_Gamma)/2
        re_op = -1/4*de_op-Sx@de_op@Sx-Sy@de_op@Sy-Sz@de_op@Sz

        Rhot = evolving_B @ Rhot @ evolving_B.T.conjugate()  # Zeeman effect

        Rhot = Rse * (alpha + 4 * alpha @ mSS - Rhot) * dt + (
                ER + de_op+re_op) * dt + Rhot
        Rhot = hyperfine * Rhot
        # -----------------Observables-----------------#
        Maz[n] = np.trace(az@Rhot)
        Max[n] = np.trace(ax@Rhot)
        # Vx = np.trace(Rhot @ ((ax + bx) @ (ax + bx) + (ay + by) @ (ay + by) + (az + bz) @ (az + bz)))
        # V[n] = Vx
        population=np.trace(Rhot)
    return Max, Maz
...
Max,Maz=master_equation(3/2,0.001,1,T1)
