...
x = np.array(pd.read_csv('data/x.csv')).flatten()
C_1z2z = np.array(pd.read_csv('data/C_1z2z.csv')).flatten()
C_1z1z  = np.array(pd.read_csv('data/C_1z1z.csv')).flatten()
T=10
dt=0.01
...
#     ax_1.plot(Jx,1/np.sqrt(2*np.pi)*np.exp(-Jx**2/2),linestyle='dashed')
    ...
    ax_2.annotate('$\sqrt{ \widetilde{\mathrm{var}} (\hat {J_x}) }$', xy=(0.3/10, 1), xytext=(2/10, 1),
