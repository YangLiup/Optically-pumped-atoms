...
def spin_operators_of_2or1_alkali_metal_atoms(N, I):
    ...
    if N == 1:
        ax = block_diag(spin_Jx(a), qzero(2 * b + 1))
        bx = block_diag(qzero(2 * a + 1), spin_Jx(b))
        ay = block_diag(spin_Jy(a), qzero(2 * b + 1))
        by = block_diag(qzero(2 * a + 1), spin_Jy(b))
        az = block_diag(spin_Jz(a), qzero(2 * b + 1))
        bz = block_diag(qzero(2 * a + 1), spin_Jz(b))
